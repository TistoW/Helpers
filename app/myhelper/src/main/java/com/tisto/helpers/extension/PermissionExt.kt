...
fun Activity.getAlertDialog(onClick: (() -> Unit)? = null) {
    ...
    builder.setTitle(this.getString(R.string.permission_warning_dialog))
    builder.setMessage(this.getString(R.string.app_need_some_permission))
    builder.setPositiveButton(this.getString(R.string.izinkan)) { dialog, _ ->
        dialog.dismiss()
        onClick?.invoke()
    }
    builder.setNegativeButton(this.getString(R.string.tutup)) { dialog, _ ->
        dialog.dismiss()
        this.finish()
    }
    builder.show()
